...
# R - register
                ...
                outline += self.fullmachinecode[i][0:11] + " " + self.fullmachinecode[i][11:16] + " " + self.fullmachinecode[i][16:22]
                outline += " " + self.fullmachinecode[i][22:27] + " " + self.fullmachinecode[i][27:32] + "\t" + str(memaddr)
                outline += "\t" + str(self.instructions[i]) + "\tR" + str(self.rdarray[i]) + ", R" + str(self.rnarray[i]) + ", "
...
# I - immediate
                ...
                outline += self.fullmachinecode[i][0:10] + " " + self.fullmachinecode[i][10:22] + " " + self.fullmachinecode[i][22:27]
                outline += " " + self.fullmachinecode[i][27:32] + "\t" + str(memaddr) + "\t" + self.instructions[i] + "\tR"
                outline += str(self.rdarray[i]) + ", R" + str(self.rnarray[i]) + ", #" + str(self.immarray[i])
# D - data load / store
            elif self.instructionformat[i] == 'D':
                outline += self.fullmachinecode[i][0:11] + " " + self.fullmachinecode[i][11:20] + " " + self.fullmachinecode[i][20:22]
                outline += " " + self.fullmachinecode[i][22:27] + " " + self.fullmachinecode[i][27:32] + "\t" + str(memaddr)
                outline += "\t" + self.instructions[i] + "\tR" + str(self.rdarray[i]) + ", [R" + str(self.rnarray[i]) + ", #"
                outline += str(self.addrarray[i]) + "]"
# IM - move immediate / multiple
            elif self.instructionformat[i] == 'IM':
                outline += self.fullmachinecode[i][0:9] + " " + self.fullmachinecode[i][9:11] + " " + self.fullmachinecode[i][11:27]
                outline += " " + self.fullmachinecode[i][27:32] + "\t" + str(memaddr) + "\t" + self.instructions[i] + "\tR"
                outline += str(self.rdarray[i]) + ", " + str(self.immarray[i]) + ", LSL " + str(self.shamtarray[i])
# B - this is the branch instruction
...
# CB - conditional branch
                ...
                outline += self.fullmachinecode[i][0:8] + " " + self.fullmachinecode[i][8:27] + " " + self.fullmachinecode[i][27:32]
                outline += "\t" + str(memaddr) + "\t" + self.instructions[i] + "\tR" + str(self.rdarray[i]) + ", #"
                outline += str(self.offsarray[i])
# BR - this is the data segment break
            elif self.instructionformat[i] == 'BR':
                outline += self.fullmachinecode[i][0:8] + " " + self.fullmachinecode[i][8:11] + " " + self.fullmachinecode[i][11:16]
                outline += " " + self.fullmachinecode[i][16:21] + " " + self.fullmachinecode[i][21:26] + " "
                outline += self.fullmachinecode[i][26:32] + "\t" + str(memaddr) + "\tBREAK"
    ...
    def emulator(self):
        ...
        while self.instructionformat[pccounter] != 'BR':
            ...
            if self.instructionformat[pccounter] == 'NOP':
                ...
                outline += str(self.instructions[pccounter]) + "\tR" + str(self.rdarray[pccounter]) + ", R" + str(self.rnarray[pccounter]) + ", "
                if self.instructions[pccounter] == 'LSL' or self.instructions[pccounter] == 'LSR' or self.instructions[pccounter] == 'ASR':
                    outline += "#" + str(self.shamtarray[pccounter])
                else:
                    outline += "R" + str(self.rmarray[pccounter])
            elif self.instructionformat[pccounter] == 'I':
                outline += self.instructions[pccounter] + "\tR" + str(self.rdarray[pccounter]) + ", R" + str(self.rnarray[pccounter]) + ", #" + str(self.immarray[pccounter])
            elif self.instructionformat[pccounter] == 'D':
                outline += self.instructions[pccounter] + "\tR" + str(self.rdarray[pccounter]) + ", [R" + str(self.rnarray[pccounter]) + ", #"
                outline += str(self.addrarray[pccounter]) + "]"
            elif self.instructionformat[pccounter] == 'IM':
                outline += self.instructions[pccounter] + "\tR"
                outline += str(self.rdarray[pccounter]) + ", " + str(self.immarray[pccounter]) + ", LSL " + str(self.shamtarray[pccounter])
...
# R -> instructions[] rdarray[] rnarray[] { rmarray[] or shamtarray[] }
            if self.instructions[pccounter] == 'ADD':
                registers[self.rdarray[pccounter]] = registers[self.rnarray[pccounter]] + registers[self.rmarray[pccounter]]
...
# D-> instructions[] rdarray[] rnarray[] addrarray[]
                ...
                jumpaddress = self.rnarray[pccounter] + self.addrarray[pccounter]
...
# IM-> instructions[] rdarray[] immarray[] shamtarray[]
                ...
                if self.shamtarray[pccounter] == 0:
                    keepvalue = self.rdarray[pccounter] & 0xFFFFFFFFFFFF0000
                elif self.shamtarray[pccounter] == 16:
                    keepvalue = self.rdarray[pccounter] & 0xFFFFFFFF0000FFFF
                elif self.shamtarray[pccounter] == 32:
                    keepvalue = self.rdarray[pccounter] & 0xFFFF0000FFFFFFFF
                elif self.shamtarray[pccounter] == 48:
                    keepvalue = self.rdarray[pccounter] & 0x0000FFFFFFFFFFFF
...
#                pccounter = (self.addrarray[pccounter] / 4) - 97
#CB-> rdarray[] offsarray[]
            elif self.instructions[pccounter] == 'CBZ' and self.rdarray[pccounter] == 0:
                pccounter += self.offsarray[pccounter]
            elif self.instructions[pccounter] == 'CBNZ' and self.rdarray[pccounter] != 0:
                pccounter += self.offsarray[pccounter]
            print "===================="
