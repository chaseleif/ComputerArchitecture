...
class Issue:
    ...
    def run(self):
        for i in range(0,3):
            if self.preissue[i] not 0:
                self.prealu[0] = self.preissue[i]
                break

# This is the MEM unit
class MEM:
    ...
    def run(self):
        if self.premem[0] is not 0:
            self.postmem = self.premem
...
class ALU:
    ...
    def run(self):
        if self.prealu[0] is not 0:
            self.postalu = self.prealu[0]
            self.prealu[0] = self.prealu[1]
            self.prealu[1] = 0
...
class WB:
    ...
    def run(self):
        if self.postmem is not 0:
            self.registers[0] = self.postmem
            self.postmem = 0
        if self.postalu is not 0:
            self.registers[0] = self.postalu
            self.postalu = 0

# This is the Cache
class Cache:
    def __init__(self, datasegment, dataendi, dacache):
...
class Emulator:
    ...
    def run(asm):
        ...
        cyclecounter = 0
        pccounter = 0
        cache = Cache(asm.datasegment, asm.dataendindex, cache)
